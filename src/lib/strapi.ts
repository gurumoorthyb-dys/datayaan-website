...
/**
 * Fetch data from Strapi with ISR (Incremental Static Regeneration)
 * @param path - API endpoint path
 * @param revalidate - Revalidation interval in seconds (default: 60)
 */
export async function fetchStrapiISR(path: string, revalidate: number = 60) {
    try {
        const url = `${STRAPI_URL}/api${path}`;

        const res = await fetch(url, {
            next: { revalidate } // ISR revalidation interval
        });

        if (!res.ok) {
            console.error("STRAPI ISR FETCH ERROR:", url, res.status, res.statusText);
            return null;
        }
        ...
        console.error("STRAPI ISR ERROR:", e);
        return null;
    }
}
